...
pub fn plan_move_set<D: MoveDomain + ?Sized>(
    ...
    let mut related_by_entity =
        domain.related_entities_for_set(&entity_ids)?;
    ...
    let mut active_leases_by_entity =
        domain.active_leases_for_set(&entity_ids)?;
    ...
    for entity_id in &entity_ids {
        ...
        let active_leases =
            active_leases_by_entity.remove(entity_id).unwrap_or_default();
...
fn build_entity_plan_with_shared_context<D: MoveDomain + ?Sized>(
    ...
    let subdir = domain.entity_subdir().to_string();
    let source_entity_path = domain.source_entity_path(entity_id)?;
    let resolved_source_path = source_entity_path.clone().unwrap_or_else(|| {
        source_store_root.join(&subdir).join(entity_id.to_string())
    });
    let destination_entity_path =
        target_store_root.join(&subdir).join(entity_id.to_string());
    ...
    for related_entity_id in inbound.iter().chain(outbound.iter()).copied() {
        let visible_from_destination = target_store_present
            && *visibility_by_id.get(&related_entity_id).unwrap_or(&false);
...
pub fn execute_move<D: MoveDomain + ?Sized>(
    ...
    execute_or_resume(domain, plan, None, false, false)
}
...
pub fn resume_move<D: MoveDomain + ?Sized>(
    ...
    execute_or_resume(domain, &plan, Some(journal), true, false)
}
...
pub fn rollback_move<D: MoveDomain + ?Sized>(
    ...
    rollback_move_within_lock(domain, journal_id, false)
}
...
fn execute_or_resume<D: MoveDomain + ?Sized>(
    ...
    let result: MoveResult<()> = (|| {
        advance_phase_planned(&mut journal, &journal_root, &span, skip_lock)?;
...
pub fn execute_move_set<D: MoveDomain + ?Sized>(
    ...
    for plan in &set_plan.entity_plans {
        lock_paths.extend(collect_lock_paths(plan.entity_id, &plan.source_store_root, &plan.target_store_root));
    }
    let mut journal = MoveSetJournal {
        ...
        entity_journal_ids: set_plan.entity_ids.iter().map(|id| (*id, Uuid::new_v4())).collect(),
...
fn execute_move_set_journal<D: MoveDomain + ?Sized>(
    ...
    for plan in &journal.entity_plans {
        if journal.completed_entity_ids.contains(&plan.entity_id) {
            ...
            if completed.phase != MoveExecutionPhase::Validated {
                release_lock_set(&journal.lock_paths);
        ...
        if existing.as_ref().is_some_and(|entry| entry.phase == MoveExecutionPhase::Validated) {
        ...
        match execute_move_within_set(domain, plan, entity_journal_id, existing, resumed) {
            ...
            Err(error) => {
                journal.entity_errors.insert(plan.entity_id, error.to_string());
                journal.failure = Some(error.to_string());
...
pub fn rollback_move_set<D: MoveDomain + ?Sized>(
    ...
    for entity_id in journal.entity_ids.iter().rev() {
        ...
        match rollback_move_within_lock(domain, entity_journal_id, true) {
            Ok(outcome) => {
            ...
            Err(error) => {
                ...
                journal.failure = Some(error.to_string());
                journal.next_recovery_step = Some("run rollback_move_set again after resolving the error".to_string());
                journal.updated_at = Utc::now();
                persist_move_set_journal(&journal.source_store_root, &journal)?;
                release_lock_set(&journal.lock_paths);
                return Err(error);
...
mod move_set_tests {
    ...
    fn make_entity(
        ...
        fs::create_dir_all(
            store_root.join("entities").join(entity_id.to_string()),
        )
    ...
    fn setup_repo() -> (tempfile::TempDir, PathBuf, PathBuf) {
        ...
        fs::create_dir_all(target_ws.join(".fixture"))
    ...
    fn plan_move_set_matches_plan_move_for_single_entity() {
        ...
        let single_plan =
            plan_move(&domain, &entity_id, &target_ws).expect("plan_move");
        let set_plan = plan_move_set(&domain, &[entity_id], &target_ws)
            .expect("plan_move_set");
    ...
    fn plan_move_set_propagates_missing_source_blocker_per_entity() {
        ...
        let set_plan =
            plan_move_set(&domain, &[present_id, missing_id], &target_ws)
                .expect("plan_move_set");
    ...
    fn execute_move_set_moves_all_entities_and_supports_per_entity_rollback() {
        ...
        let set_plan = plan_move_set(&domain, &[entity_a, entity_b], &target_ws)
            .expect("plan_move_set");
        assert!(set_plan.supported());

        let outcome =
            execute_move_set(&domain, &set_plan).expect("execute_move_set");
        assert_eq!(outcome.entity_ids, set_plan.entity_ids);
        assert_eq!(outcome.entity_outcomes.len(), 2);
        let loaded_set_journal = load_move_set_journal(
    ...
    fn resume_move_set_skips_completed_entities() {
        ...
        let domain = FixtureDomain {
            source_store_root: source_ws.join(".fixture"),
            fail_scan_number: Cell::new(Some(3)),
            scan_calls: Cell::new(0),
        };
        let entity_a = Uuid::new_v4();
        let entity_b = Uuid::new_v4();
        ...
        execute_move_set(&domain, &set_plan)
            .expect_err("second entity must interrupt the set move");
        let journal_path = fs::read_dir(
            domain.source_store_root.join(MOVE_JOURNALS_DIR),
        )
        ...
        let journal_id = Uuid::parse_str(
            journal_path.file_name().expect("name").to_string_lossy()
        ...
        assert_eq!(interrupted.completed_entity_ids.len(), 1);

        domain.fail_scan_number.set(None);
        let resumed = resume_move_set(&domain, journal_id)
            .expect("resume only unfinished entity");
        assert_eq!(resumed.entity_outcomes.len(), 2);
        assert!(resumed.entity_outcomes.iter().any(|outcome| {
            outcome.journal.entity_id == entity_a && outcome.resumed
        }));
    ...
    fn rollback_move_set_is_idempotent() {
        ...
        make_entity(&domain.source_store_root, entity_a);
        make_entity(&domain.source_store_root, entity_b);
        let plan = plan_move_set(&domain, &[entity_a, entity_b], &target_ws)
            .expect("plan_move_set");
        let moved = execute_move_set(&domain, &plan).expect("execute_move_set");

        let rolled_back = rollback_move_set(&domain, moved.journal.id)
        ...
        let repeated = rollback_move_set(&domain, moved.journal.id)
